...
export const BRANDS = [
  "Tesla", "BMW", "Mercedes-Benz", "Audi", "Porsche", "Toyota",
  "Honda", "Hyundai", "Kia", "Volkswagen", "Volvo", "Lexus",
  "Ford", "Mazda", "Nissan", "Polestar",
];
...
export function generateMockListings(): Listing[] {
  return CARS.map((c, i): Listing => {
    ...
    const images = [...SAMPLE_IMAGES.slice(startIdx), ...SAMPLE_IMAGES.slice(0, startIdx)].slice(0, 10);
    return {
      id: `seed-${i + 1}`,
      ...
      bodyType: c.bodyType!,
      images,
      status: i < 8 ? "listed" : i < 11 ? "approved" : i < 13 ? "pending_review" : "under_inspection",
      pricing: i < 8 ? {
        basePrice: expectedPrice, refurbishment: refurb, repair, transportation: transport,
        inspection, documentation, commission, margin, finalPrice,
      } : undefined,
...
export const SEED_TICKETS: Ticket[] = [
  { id: "t-1", name: "John Lee", email: "john@example.com", subject: "Financing options for Tesla Model 3", category: "Financing", message: "Hi, what are the EMI options?", status: "open", createdAt: Date.now() - 86400000 },
  { id: "t-2", name: "Maria Gomez", email: "maria@example.com", subject: "Inspection report missing", category: "Inspection", message: "Can you share the full report?", status: "in_progress", createdAt: Date.now() - 2 * 86400000 },
  { id: "t-3", name: "Sam Patel", email: "sam@example.com", subject: "Refund status", category: "Purchase", message: "Awaiting refund update.", status: "waiting_customer", createdAt: Date.now() - 4 * 86400000 },
  { id: "t-4", name: "Aisha Khan", email: "aisha@example.com", subject: "Doc upload error", category: "Support", message: "Can't upload PDF.", status: "resolved", createdAt: Date.now() - 7 * 86400000 },
];
...
export function calculateFinalPrice(p: { basePrice: number; refurbishment: number; repair: number; transportation: number; inspection: number; documentation: number; commission: number; margin: number; }) {
  return p.basePrice + p.refurbishment + p.repair + p.transportation + p.inspection + p.documentation + p.commission + p.margin;
}
